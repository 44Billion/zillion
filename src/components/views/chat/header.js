...
f('z-chat-header', ({ h, props }) => {
  ...
  useTask(({ track }) => {
    ...
    if (element) element.querySelector('[role="menuitem"]')?.focus({ preventScroll: true })
  }, { after: 'rendering' })
...
? h`
        <div class="chat-menu" id=${view.menuId} role="menu" aria-label=${t('Chat options')} ref=${menu.floatingRef$} style=${menu.floatingStyle$()} onkeydown=${view.key}>
          ${FUTURE_FEATURES_ENABLED && !person.self ? h`<button class="chat-attention-option" type="button" role="menuitem" aria-disabled="true"><icon-bolt props=${{ size: '20px', weight: 'regular' }} /><span>${t('Get attention')}</span></button>` : null}
