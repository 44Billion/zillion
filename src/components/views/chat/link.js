...
f('z-chat-link', ({ h, props }) => {
  ...
  useTask(({ track, cleanup }) => {
    ...
    if (view.loadedFor !== identity || known) view.clear()
    ...
    const resolve = async () => {
      ...
      pending = true
      try {
        if (reference && !await canPreviewNostrReference(reference, {
        ...
        const metadata = await previews.load(url, { signal: controller.signal })
        if (controller.signal.aborted || !metadata || (reference && !metadata.found)) return
        view.metadata$(metadata)
        await Promise.all([
          (async () => {
            const source = await mediaCache.resolveImage(metadata.icon, { signal: controller.signal })
            if (!controller.signal.aborted && source) view.icon$(source)
          })(),
          (async () => {
            if (!metadata.image) return
            const source = await mediaCache.resolveImage(metadata.image, { signal: controller.signal })
  ...
  return h`<span class="chat-link"><style>${`
    z-chat-link .chat-link {
      ...
      .preview-image { display: block; width: 320px; max-width: 100%; max-height: 200px; object-fit: contain; border-radius: 6px; margin-bottom: 7px; }
    }
  `}</style><a class="reference-link" href=${href} title=${label} aria-label=${label} target="_blank" rel="noopener noreferrer">${view.icon$() ? h`<img class="reference-icon" src=${view.icon$()} alt="" referrerpolicy="no-referrer" onerror=${iconError}>` : null}<span class="reference-label">${reference ? shortNostrLabel(label) : shortUrlLabel(label, item.url.ext)}</span></a>${metadata?.title || metadata?.description || metadata?.image ? h`<a class="website-preview" href=${href} target="_blank" rel="noopener noreferrer">${view.image$() ? h`<img class="preview-image" src=${view.image$()} alt="" referrerpolicy="no-referrer" onerror=${() => view.image$(null)}>` : null}<small class="preview-site">${metadata.site}</small>${metadata.title ? h`<strong>${metadata.title}</strong>` : null}${metadata.description ? h`<span class="preview-description">${metadata.description}</span>` : null}</a>` : null}</span>`
})
