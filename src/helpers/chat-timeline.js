...
export function chatTimeline (events, { locale, now = Date.now(), t = value => value } = {}) {
  ...
  return events.map(event => {
    ...
    const message = {
      attachment: messageAttachment(event), localSource: event.localSource, id: event.id, text: compactWhitespace(event.content), real: true, outgoing: true, status: event.status ?? 'saved',
