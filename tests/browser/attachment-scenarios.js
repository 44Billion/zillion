...
export async function checkAttachmentScenarios ({ browser, evaluate, origin, requests = [] }) {
  ...
  try {
    ...
    await browser.until(() => evaluate('document.querySelector(".route-page[data-active=true] .viewer-asset[data-current=true] img")?.naturalWidth === 1'), 'original local image in viewer')
    assert.equal(await evaluate('new URL(document.querySelector(".route-page[data-active=true] .viewer-asset[data-current=true] img").src).origin'), 'https://nostr.alt', 'viewer reads the original virtual file, not its blob thumbnail')
    ...
    await browser.until(() => evaluate('document.querySelector(".route-page[data-active=true] .viewer-asset[data-current=true] video")?.videoWidth === 32'), 'original local video in viewer')
    await evaluate('document.querySelector(".route-page[data-active=true] .viewer-close").click()')
