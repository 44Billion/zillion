...
test('profile preview, local controls, editing, identity sharing and retained chat navigation', { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await ready('.home')
    await evaluate('profileTest.setOwn({ display_name: "Own display", name: "own", nip05: "own@example.com", lud16: "own@example.com", about: "Account biography" })')
    ...
    assert.equal(await read('.profile-name'), 'own')
    ...
    const bitcoin = await evaluate(`document.querySelector('${active}[data-identifier=bitcoin] .identifier-text').title`)
    assert.ok(bitcoin.startsWith('bc1p'))
    for (const id of ['npub', 'lightning', 'bitcoin']) {
      await shareMode('copy')
      await click(`[data-identifier=${id}] .profile-share`)
      await browser.until(() => evaluate(`document.querySelector('${active}[data-identifier=${id}] .profile-share').dataset.copied === 'true'`), `${id} copy feedback`)
      assert.deepEqual(await evaluate('copies'), [await evaluate(`document.querySelector('${active}[data-identifier=${id}] .identifier-text').title`)])
      assert.equal(await evaluate(`document.querySelectorAll('${active}.profile-share[data-copied=true]').length`), 1)
    }
    await evaluate('profileTest.setOwn({ ...profileTest.profile(), name: " \t " })')
    await browser.until(async () => await read('.profile-name') === 'Own display', 'profile name fallback')
    ...
    assert.equal(await evaluate(`document.querySelector('${active}input[name=bitcoin]').readOnly`), true)
    assert.equal(await evaluate(`document.querySelector('${active}input[name=bitcoin]').value`), bitcoin)
    assert.equal(await evaluate(`document.querySelector('${active}input[name=lightning]').value`), 'own@example.com')
    const setLightning = value => evaluate(`(() => { const input = document.querySelector('${active}input[name=lightning]'); input.value = ${JSON.stringify(value)}; input.dispatchEvent(new Event('input', { bubbles: true })); })()`)
    await setLightning('lnurlw://example.com/not-pay')
    await browser.until(() => evaluate(`document.querySelector('${active}input[name=lightning]').getAttribute('aria-invalid') === 'true'`), 'non-pay Lightning validation')
    await setLightning('lnurlp://example.com/pay?token=draft')
    await browser.until(() => evaluate(`document.querySelector('${active}input[name=lightning]').getAttribute('aria-invalid') === 'false'`), 'pay Lightning draft accepted')
    assert.equal(await evaluate(`Boolean(document.querySelector('${active}input[name=display_name]'))`), false)
    assert.equal(await evaluate(`document.querySelector('${active}input[name=name]').value`), 'Own display')
    await evaluate('profileTest.setOwn({ ...profileTest.profile(), name: "own" })')
    await browser.until(() => evaluate(`document.querySelector('${active}input[name=name]').value === 'own'`), 'untouched name follows metadata')
    await evaluate(`(() => { const input = document.querySelector('${active}input[name=name]'); input.value = ''; input.dispatchEvent(new Event('input', { bubbles: true })); })()`)
    await evaluate('new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))')
    assert.equal(await evaluate(`document.querySelector('${active}input[name=name]').value`), '')
    await evaluate(`(() => { const input = document.querySelector('${active}input[name=name]'); input.value = 'Local draft'; input.dispatchEvent(new Event('input', { bubbles: true })); })()`)
    ...
    await ready('.profile-editor')
    assert.equal(await evaluate(`document.querySelector('${active}input[name=name]').value`), 'Local draft')
    assert.equal(await evaluate('profileTest.profile().display_name'), 'Own display')
    assert.equal(await evaluate('profileTest.profile().name'), 'own')
    assert.equal(await evaluate('profileTest.profile().lud16'), 'own@example.com')
    assert.equal(await evaluate(`document.querySelector('${active}input[name=lightning]').value`), 'lnurlp://example.com/pay?token=draft')
    ...
    await click('[data-identifier=lightning] .profile-share')
    await browser.until(() => evaluate('shared.length === 2'), 'raw Lightning share')
    assert.deepEqual(await evaluate('shared[1]'), { text: 'lnurlp://example.com/lnurlp/luna' })
    ...
    await evaluate('profileTest.setOwn({ display_name: "Arthur", name: "arthur", nip05: "arthur@example.com", lud16: "arthur@example.com", about: "Fotografia, tecnologia e boas conversas." }); profileTest.locale("pt-BR")')
    ...
    const longLightning = 'lnurlp://example.com/pay?token=' + 'verylongtoken'.repeat(30)
    await evaluate(`profileTest.setOwn({ nip05: ${JSON.stringify(longIdentifier)}, lud06: ${JSON.stringify(longLightning)} })`)
    ...
    await shareMode('copy')
    await click('[data-identifier=lightning] .profile-share')
    await browser.until(() => evaluate('copies.length === 1'), 'long Lightning copied')
    assert.equal(await evaluate('copies[0]'), longLightning)
    const columns = await evaluate(`Array.from(document.querySelectorAll('${active}.profile-identifier'), row => ({ icon: row.querySelector('.identifier-icon').getBoundingClientRect().left, action: row.querySelector('.profile-share').getBoundingClientRect().left }))`)
    assert.equal(new Set(columns.map(row => row.icon)).size, 1)
    assert.equal(new Set(columns.map(row => row.action)).size, 1)
