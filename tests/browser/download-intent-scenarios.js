...
export async function checkDownloadIntent ({ browser, evaluate, origin, downloads, photo, videoTags, png, videoBytes }) {
  ...
  try {
    ...
    await browser.until(() => evaluate(`(${videoRow}).querySelector('.download-media img:not(.attachment-placeholder)')?.naturalWidth === 32`), 'download video poster')
    assert.equal(await evaluate(`(${videoRow}).querySelector('video')`), null, 'download-only video never opens a player')
    await download(`(${videoRow}).querySelector('a.download-media')`, 'clip.webm', Buffer.from(videoBytes))
    assert.equal(await evaluate(`(${videoRow}).querySelector('video')`), null)
